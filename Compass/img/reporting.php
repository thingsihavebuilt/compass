<?php
if($_REQUEST['groupr']):unset($_REQUEST['staffr']);
elseif($_REQUEST['roler']):unset($_REQUEST['groupr']);
elseif($_REQUEST['staffr']):unset($_REQUEST['groupr']);unset($_REQUEST['roler']);
endif;?>
...
?>
<div id="dual_x_div_<?php echo 1;?>" style="width: 100%; height: 500px;"></div>

<script type="text/javascript">
google.setOnLoadCallback(drawStuff_<?php echo 1;?>);
...
echo '<td class="bg-grade-'.$i.'"><label><input  type="radio" class="pd" onclick="return false;" name="'.str_replace(' ','',$fi).'['.$post->ID.']" value="'.$v.'" '.$checked.' />'.$op.'</label></td>';
$i++;
endforeach;
?>
</tr>
      ...
      function drawStuff_<?php echo $post->ID;?>() {
        var data = new google.visualization.arrayToDataTable([
          ['Competency', 'Required Score', 'Actual'],
		    ...
		    foreach($jobcompetencies as $jc => $v):
			if($v != ''):
			echo " ['".get_the_title($jc)."', ".$v.", ".$competencies[$jc]."],";
			endif;
			endforeach;
			?>
...
<script type="text/javascript">
google.setOnLoadCallback(drawStuff_<?php echo $user->ID;?>);
...
?>
<div id="dual_x_div" style="width: 100%; height: 500px; background:#fff; padding:0%"></div>
      ...
      function drawStuff() {
        var data = new google.visualization.arrayToDataTable([
          ['Competency', 'Required Score', 'Average'],
		    ...
		    foreach($jobcompetencies as $jc => $v):
			if($v != ''):
			echo " ['".get_the_title($jc)."', ".$v.", ".ceil($groupcompetencies[$jc]/$groupcompetenciescount[$jc])."],";
			endif;
			endforeach;
			?>
